...
def createCareunitClean(con, schemaName):
    log.info("Creating table: " + schemaName + ".lk_trans_careunit_clean")
    dropQuery = """drop table if exists """ + schemaName + """.lk_trans_careunit_clean cascade"""
    createQuery = """CREATE TABLE """ + schemaName + """.lk_trans_careunit_clean AS
        SELECT
...
def createCaresiteCdm(con, schemaName):
    log.info("Creating table: " + schemaName + ".cdm_care_site")
    dropQuery = """drop table if exists """ + schemaName + """.cdm_care_site cascade"""
    createQuery = """CREATE TABLE """ + schemaName + """.cdm_care_site
        (
        ...
        ;
        """
    insertQuery = """INSERT INTO """ + schemaName + """.cdm_care_site
        SELECT
        ...
        FROM 
        """ + schemaName + """.lk_trans_careunit_clean src
        LEFT JOIN
...
def migrate(con, schemaName):
    createCareunitClean(con = con, schemaName = schemaName)
    createCaresiteCdm(con = con, schemaName = schemaName)
